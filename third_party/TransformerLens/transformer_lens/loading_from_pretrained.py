...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    elif "pixtral" in official_model_name.lower() or "llava" in official_model_name.lower():
    ...
    if official_model_name.startswith(
    ...
    elif "Llama-3.2-11B" in official_model_name:
        cfg_dict = {
            "d_model": 4096,
            "d_head": 128,
            "n_heads": 32,
            ...
            "n_ctx": 2048,  # capped due to memory issues
            "eps": 1e-5,
            # "d_vocab": 128264, # OR 128256 ???
            "d_vocab": 128264,
            "d_vocab_out": 128256,
            "act_fn": "silu",  # FOR TEXT MODEL
            "n_key_value_heads": 8,
            "normalization_type": "RMS",
            "positional_embedding_type": "rotary",
            "rotary_adjacent_pairs": False,
            ...
            "final_rms": True,
            "gated_mlp": True,
            "cross_attn_blocks": [3, 8, 13, 18, 23, 28, 33, 38],
        }
    elif "Pixtral-12B" in official_model_name:
        cfg_dict = {
            "d_model": 5120,
            "d_head": 4096
            // 32,  # 4096 is the hidden size of the head dim (unified across heads, which is DIFFERENT from the d_model)
            "n_heads": 32,
            "n_key_value_heads": 8,
            "d_mlp": 14336,
            "n_layers": 40,
    ...
    elif "llava-hf/llava-1.5-7b-hf" in official_model_name:
        cfg_dict = {
            "d_model": 4096,
            "d_head": 128,
            "n_heads": 32,
        ...
        cfg_dict = {
            ...
            "window_size": 1024,  #
            "use_local_attn": True,  #
            "attn_types": ["local", "local", "local", "local", "local", "global"] * 8,  #
            # "attn_scores_soft_cap": 50.0, #
            # "output_logits_soft_cap": 30.0, #
            "gated_mlp": True,  #
            "final_rms": True,  #
    ...
    if (
        ...
        if cfg.original_architecture == "GPT2LMHeadModel":
            ...
            if "pixtral" in cfg.model_name.lower():
                ...
                state_dict = convert_llava_1_5_weights(hf_model, cfg)
            else:
                raise ValueError(
                    f"Wierd combination of arch {cfg.original_architecture} and model name {cfg.model_name}"
                )
        else:
            raise ValueError(
