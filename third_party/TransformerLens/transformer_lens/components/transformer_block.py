...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, cfg: Union[Dict, HookedTransformerConfig], block_index):
        ...
        qk_norm_cls = None
        if self.cfg.use_qk_norm is not None:
            qk_norm_cls = normalization_layer
        ...
        if not self.cfg.use_local_attn:
            ...
            self.attn = attention(self.cfg, attn_type, block_index, qk_norm_cls=qk_norm_cls)
        if not self.cfg.attn_only:
    ...
    def forward(
        ...
        overriding_attention_mask: Optional[Int[torch.Tensor, "batch 1 kv_pos kv_pos"]] = None,
