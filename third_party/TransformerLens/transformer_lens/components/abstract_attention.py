...
class AbstractAttention(ABC, nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            force_ignore_pos_embed (bool, optional): If True, don't apply positional embeddings. Defaults to False.
        """
        super().__init__()
        ...
        self.force_ignore_pos_embed = force_ignore_pos_embed
        ...
        if self.cfg.positional_embedding_type == "shortformer" and not self.force_ignore_pos_embed:
        ...
        elif self.cfg.positional_embedding_type == "rotary" and not self.force_ignore_pos_embed:
            ...
            if self.cfg.thw_rotary:
                sin = sin.view(1, 1, sin.shape[0], sin.shape[1]).repeat(3, 1, 1, 1)
                cos = cos.view(1, 1, cos.shape[0], cos.shape[1]).repeat(3, 1, 1, 1)
            self.register_buffer("rotary_sin", sin)
            self.register_buffer("rotary_cos", cos)
        elif self.cfg.positional_embedding_type == "alibi" and not self.force_ignore_pos_embed:
            ...
            and not self.force_ignore_pos_embed
        ):
    ...
    def forward(
        ...
        overriding_attention_mask: Optional[Float[torch.Tensor, "batch 1 kv_pos kv_pos"]] = None,
        ...
        overriding_attention_mask is an optional mask to replace the attention weights. Defaults to None.
        ...
        if self.cfg.positional_embedding_type == "rotary" and not self.force_ignore_pos_embed:
            rotary_func = self.apply_qwen_rotary if self.cfg.thw_rotary else self.apply_rotary
            q = self.hook_rot_q(rotary_func(q, kv_cache_pos_offset, attention_mask))
            k = self.hook_rot_k(rotary_func(k, 0, attention_mask))  # keys are cached so no offset
        ...
        if self.cfg.positional_embedding_type == "alibi" and not self.force_ignore_pos_embed:
        ...
        if overriding_attention_mask is not None:
            attn_scores = torch.where(overriding_attention_mask.to(attn_scores.device), attn_scores, self.IGNORE)
        else:
    ...
    def apply_qwen_rotary(
        ...
        """
        Taken from transformers.modeling_qwen2_vl.apply_multimodal_rotary_pos_emb.
        ...
        """
        assert attention_mask is None and past_kv_pos_offset == 0, "Not supported yet"
        ...
        mrope_section = self.cfg.thw_rotary_pe_sections * 2

        # Grab only the relevant positions
        cos = self.rotary_cos[:, :, : x.shape[1], :]
        sin = self.rotary_sin[:, :, : x.shape[1], :]

        # THW-RopE magic (see explanation above)
        cos = torch.cat(
