...
def convert_llama_weights(llama, cfg: HookedTransformerConfig):
    ...
    # state_dict[f"pos_embed"] = llama.model.rotary_emb(torch.ones(1, device=cfg.device, dtype=cfg.dtype), 
    #                                                   torch.arange(10000).unsqueeze(0)) # HACK to get the positional embeddings

    for l in range(cfg.n_layers):
        # HACK to get the positional embeddings from HF model (theres currently a bug in the TLens implementation)
        pos_embed = llama.model.rotary_emb(torch.ones(1, device=llama.device, dtype=cfg.dtype), 
                                                            torch.arange(cfg.n_ctx, device=llama.device).unsqueeze(0))
        cos, sin = pos_embed
